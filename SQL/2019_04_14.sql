...
FROM course c
JOIN score sc ON sc.cid = c.id
JOIN student s ON s.id = sc.sid
GROUP BY c.id,c.name

-- 33. 成绩不重复，查询选修「张三」老师所授课程的学生中，成绩最高的学生信息及其成绩
SELECT s.*,c.name,sc.cid,sc.score
FROM student s,score sc,course c,teacher t
WHERE s.id = sc.sid AND sc.cid = c.id AND c.tid = t.id and t.name = '张三'
ORDER BY sc.score desc 
LIMIT 1


-- 34. 成绩有重复的情况下，查询选修「张三」老师所授课程的学生中，成绩最高的学生信息及其成绩
SELECT t.name,s.id,s.name,sc.score
FROM teacher t,student s,course c,score sc
WHERE t.id = c.tid AND c.id = sc.cid AND sc.sid = s.id
AND t.id = 1 AND sc.score = (SELECT MAX(score) FROM score ss WHERE ss.cid = sc.cid)
...
FROM score sc,(
    ...
    GROUP BY cid,score
    HAVING COUNT(*) > 1) n
WHERE sc.cid = n.cid AND n.score = sc.score 

-- 36. 查询每门成绩最好的前两名
SELECT *
FROM
	(SELECT s.name,sc.sid,sc.cid,
	ROW_NUMBER() OVER(PARTITION BY cid ORDER BY score DESC) rn
	FROM score sc
	JOIN student s ON s.id = sc.sid) AS tmp 
WHERE rn < 3;
/*row_number()over(partition by col1 order by col2)函数
根据col1分组，在分组内部根据col2排序，而此函数计算的值就表示每组内部排序后的顺序编号（组内是连续且唯一的）*/

-- 37. 统计每门课程的学生选修人数（超过 5 人的课程才统计）。
SELECT sc2.cid,COUNT(*) AS ct
FROM score sc2
GROUP BY sc2.cid
HAVING COUNT(*) >= 5

-- 38. 检索至少选修两门课程的学生学号
SELECT sc2.sid,COUNT(*) as cn
FROM score sc2
GROUP BY sc2.sid
HAVING COUNT(*) > 1
...
SELECT s.name,
	...
	END)
FROM student s
/*curdate : 獲取當前日期(2008-12-29)
 curtime() : 獲取當前時間(16:52:32) 
NOW():獲取當前日期時間資訊(2008-12-29 16:25:46)
DayOfMonth():
	計算傳入日期的是這個月的第幾天
	天數 = DayOfMonth(日期)
	回傳數值的範圍從1到31
...
例如 2010-3-14 ，礼拜天 
SELECT YEARWEEK('2010-3-14') 返回 11 表示今年第11週
SELECT YEARWEEK('2010-3-14',1) 返回 10  表示今年第10週
*/
